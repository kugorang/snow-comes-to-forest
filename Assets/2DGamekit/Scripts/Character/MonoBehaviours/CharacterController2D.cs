...
namespace Gamekit2D
{
    ...
    {
        private readonly RaycastHit2D[] m_FoundHits = new RaycastHit2D[3];
        private readonly Collider2D[] m_GroundColliders = new Collider2D[3];
        private readonly RaycastHit2D[] m_HitBuffer = new RaycastHit2D[5];
        private readonly Vector2[] m_RaycastPositions = new Vector2[3];
        ...
        {
            var delta = position - m_CurrentPosition;
        ...
        /// <summary>
        ///     This updates the state of IsGrounded.  It is called automatically in FixedUpdate but can be called more frequently
        ///     if higher accurracy is required.
        /// </summary>
        public void CheckCapsuleEndCollisions(bool bottom = true)
        {
            ...
            {
                raycastStart = Rigidbody2D.position + m_Capsule.offset;
                raycastDistance = m_Capsule.size.x * 0.5f + groundedRaycastDistance * 2f;

                if (bottom)
                {
                    raycastDirection = Vector2.down;
                    var raycastStartBottomCentre =
                        raycastStart + Vector2.down * (m_Capsule.size.y * 0.5f - m_Capsule.size.x * 0.5f);
                ...
                {
                    raycastDirection = Vector2.up;
                    var raycastStartTopCentre =
                        raycastStart + Vector2.up * (m_Capsule.size.y * 0.5f - m_Capsule.size.x * 0.5f);
            ...
            for (var i = 0; i < m_RaycastPositions.Length; i++)
            {
                var count = Physics2D.Raycast(m_RaycastPositions[i], raycastDirection, m_ContactFilter, m_HitBuffer,
                    raycastDistance);
                ...
                else
                {
                    ...
                    foreach (var t in m_HitBuffer)
                    {
                        if (t.collider == null) continue;
                        if (!PhysicsHelper.ColliderHasPlatformEffector(t.collider)) IsCeilinged = true;
                    }
                }
            }
            ...
            if (bottom)
            {
                ...
                for (var i = 0; i < m_FoundHits.Length; i++)
                ...
                if (hitCount > 0) groundNormal.Normalize();

                var relativeVelocity = Velocity;
                for (var i = 0; i < m_GroundColliders.Length; i++)
                {
                    ...
                    if (m_Capsule != null)
                        if (m_GroundColliders[1] != null)
                        {
                            var capsuleBottomHeight =
                                Rigidbody2D.position.y + m_Capsule.offset.y - m_Capsule.size.y * 0.5f;
                            var middleHitHeight = m_FoundHits[1].point.y;
                            IsGrounded &= middleHitHeight < capsuleBottomHeight + groundedRaycastDistance;
                        }
                }
            }

            for (var i = 0; i < m_HitBuffer.Length; i++) m_HitBuffer[i] = new RaycastHit2D();
        }
    }
}
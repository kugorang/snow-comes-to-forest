...
namespace Cinemachine.Utility
{
    ...
    public static class UnityVectorExtensions
    {
        ...
        {
            var s = s1 - s0;
            var len2 = Vector3.SqrMagnitude(s);
            if (len2 < Epsilon)
        ...
        {
            var s = s1 - s0;
            var len2 = Vector2.SqrMagnitude(s);
            if (len2 < Epsilon)
                return 0; // degenrate segment
        ...
        /// <param name="refNormal">
        ///     This is needed in order to determine the sign.
        ...
        public static float SignedAngle(Vector3 from, Vector3 to, Vector3 refNormal)
        {
            from.Normalize();
            to.Normalize();
            var dot = Vector3.Dot(Vector3.Cross(from, to), refNormal);
            ...
            Vector3 vA, Vector3 vB, float t, Vector3 up)
        {
            ...
            var dirA = vA / dA;
            var dirB = vB / dB;
            var qA = Quaternion.LookRotation(dirA, up);
            var qB = Quaternion.LookRotation(dirB, up);
            var q = UnityQuaternionExtensions.SlerpWithReferenceUp(qA, qB, t, up);
    ...
    public static class UnityQuaternionExtensions
    {
            ...
            Quaternion qA, Quaternion qB, float t, Vector3 up)
        {
            ...
            var qA1 = Quaternion.Inverse(qBase) * qA;
            var qB1 = Quaternion.Inverse(qBase) * qB;
        ...
        {
            var v = new Vector4(q.x, q.y, q.z, q.w).normalized;
            ...
            this Quaternion orient, Vector3 lookAtDir, Vector3 worldUp)
        {
            ...
            var angleV = UnityVectorExtensions.SignedAngle(

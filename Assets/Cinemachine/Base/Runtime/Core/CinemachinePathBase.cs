...
namespace Cinemachine
{
    ...
    public abstract class CinemachinePathBase : MonoBehaviour
    {
        ...
        [Tooltip("Path samples per waypoint.  This is used for calculating path distances.")] [Range(1, 100)]
            ...
            Vector3 p, int startSegment, int searchRadius, int stepsPerSegment)
        {
            ...
            var stepSize = 1f / stepsPerSegment;
            float bestPos = startSegment;
            var bestDistance = float.MaxValue;
            var iterations = stepsPerSegment == 1 ? 1 : 3;
            for (var i = 0; i < iterations; ++i)
            {
                var v0 = EvaluatePosition(start);
                for (var f = start + stepSize; f <= end; f += stepSize)
                {
                    var v = EvaluatePosition(f);
                    var t = p.ClosestPointOnSegment(v0, v);
                    var d = Vector3.SqrMagnitude(p - Vector3.Lerp(v0, v, t));
                    if (d < bestDistance)
                    {
                        bestDistance = d;
        ...
        /// <summary>
        ///     Normalize a distance along the path based on the path length.
        ///     If the distance cache is not valid, then calling this will
        ///     trigger a potentially costly regeneration of the path distance cache
        /// </summary>
        ...
        public float GetPathPositionFromDistance(float distance)
        {
            ...
            var d = distance / m_cachedDistanceStepSize;
            var i = Mathf.FloorToInt(d);
        ...
        private void ResamplePath(int stepsPerSegment)
        {
            ...
            var minPos = MinPos;
            var maxPos = MaxPos;
            var stepSize = 1f / Mathf.Max(1, stepsPerSegment);

            // Sample the positions
            var numKeys = Mathf.RoundToInt((maxPos - minPos) / stepSize) + 1;
            m_PosToDistance = new float[numKeys];
            m_CachedSampleSteps = stepsPerSegment;
            m_cachedPosStepSize = stepSize;

            var p0 = EvaluatePosition(0);
            m_PosToDistance[0] = 0;
            var pos = minPos;
            for (var i = 1; i < numKeys; ++i)
            {
                pos += stepSize;
                var p = EvaluatePosition(pos);
                var d = Vector3.Distance(p0, p);
                m_PathLength += d;
                p0 = p;
            ...
            if (numKeys > 1)
            {
                ...
                {
                    distance += stepSize;
                    var d = m_PosToDistance[posIndex];
                    while (d < distance && posIndex < numKeys - 1)
                        d = m_PosToDistance[++posIndex];
                    var d0 = m_PosToDistance[posIndex - 1];
                    var delta = d - d0;
        ...
        public class Appearance
        {
            ...
            [Tooltip("The width of the railroad-tracks that are drawn to represent the path")] [Range(0f, 10f)]

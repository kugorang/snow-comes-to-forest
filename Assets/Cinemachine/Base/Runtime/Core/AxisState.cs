...
namespace Cinemachine
{
    ...
    public struct AxisState
    {
        ...
        ///     If <b>true</b>, values commanded greater
        ///     than mMaxValue or less than mMinValue will wrap around.
        ///     If <b>false</b>, the value will be clamped within the range.
        /// </param>
        ...
        /// <returns>
        ///     Returns <b>true</b> if this axis' input was non-zero this Update,
        ///     <b>flase</b> otherwise
        /// </returns>
        public bool Update(float deltaTime)
        {
            ...
            var input = m_InputAxisValue;
            if (m_InvertAxis)
                input *= -1f;

            if (m_MaxSpeed > Epsilon)
            {
                var targetSpeed = input * m_MaxSpeed;
                ...
                {
                    // Need to decelerate
                    var a = Mathf.Abs(targetSpeed - mCurrentSpeed) / Mathf.Max(Epsilon, m_DecelTime);
                    var delta = Mathf.Min(a * deltaTime, Mathf.Abs(mCurrentSpeed));
                    mCurrentSpeed -= Mathf.Sign(mCurrentSpeed) * delta;
                }
                else
                {
                    // Accelerate to the target speed
                    var a = Mathf.Abs(targetSpeed - mCurrentSpeed) / Mathf.Max(Epsilon, m_AccelTime);
            ...
            var isOutOfRange = Value > mMaxValue || Value < mMinValue;
            if (isOutOfRange)
            {
                if (mWrapAround)
                {
        ...
        {
            var range = mMaxValue - mMinValue;
            if (!mWrapAround && range > 0)
            {
                var threshold = range / 10f;
                if (mCurrentSpeed > 0 && mMaxValue - Value < threshold)
                {
                    var t = (mMaxValue - Value) / threshold;
                    return Mathf.Lerp(0, m_MaxSpeed, t);
                }

                if (mCurrentSpeed < 0 && Value - mMinValue < threshold)
                {
                    var t = (Value - mMinValue) / threshold;
                    return Mathf.Lerp(0, m_MaxSpeed, t);
                }
            }

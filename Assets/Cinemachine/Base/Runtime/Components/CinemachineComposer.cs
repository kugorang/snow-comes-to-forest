...
namespace Cinemachine
{
    ...
    public class CinemachineComposer : CinemachineComponentBase
    {
        ...
        public override void MutateCameraState(ref CameraState curState, float deltaTime)
        {
            ...
            var targetDistance = (TrackedPoint - curState.CorrectedPosition).magnitude;
            ...
            else
            {
                ...
                var radHFOV = 2 * Math.Atan(Math.Tan(fov * Mathf.Deg2Rad / 2) * curState.Lens.Aspect);
                fovH = (float) (Mathf.Rad2Deg * radHFOV);
            }

            var rigOrientation = curState.RawOrientation;
            var softGuideFOV = ScreenToFOV(SoftGuideRect, fov, fovH, curState.Lens.Aspect);
            if (deltaTime < 0)
            {
                // No damping, just snap to central bounds, skipping the soft zone
                var rect = new Rect(softGuideFOV.center, Vector2.zero); // Force to center
            ...
            else
            {
                ...
                var dir = m_LookAtPrevFrame - (m_CameraPosPrevFrame + curState.PositionDampingBypass);
                if (dir.AlmostZero())
                {
                ...
                var hardGuideFOV = ScreenToFOV(HardGuideRect, fov, fovH, curState.Lens.Aspect);
        ...
        private Rect ScreenToFOV(Rect rScreen, float fov, float fovH, float aspect)
        {
            ...
            var persp = Matrix4x4.Perspective(fov, aspect, 0.01f, 10000f).inverse;

            var p = persp.MultiplyPoint(new Vector3(0, r.yMin * 2f - 1f, 0.1f));
            p.z = -p.z;
            var angle = UnityVectorExtensions.SignedAngle(Vector3.forward, p, Vector3.left);
        ...
        {
            var targetDir = TrackedPoint - state.CorrectedPosition;
            ...
            var min = (screenRect.yMin - 0.5f) * fov;
            var max = (screenRect.yMax - 0.5f) * fov;
            if (rotToRect.x < min)
                rotToRect.x -= min;
...
// that it would be damped
        ...
        {
            var angle = Vector3.Angle(dir, up);
            var halfFov = fov / 2f + 1; // give it a little extra to accommodate precision errors
            if (angle < halfFov)
            {
                // looking up
                var maxY = 1f - (halfFov - angle) / fov;
                if (r.yMax > maxY)
                {

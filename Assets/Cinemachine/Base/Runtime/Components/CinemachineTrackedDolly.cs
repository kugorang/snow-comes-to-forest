...
namespace Cinemachine
{
    ...
    public class CinemachineTrackedDolly : CinemachineComponentBase
    {
        ...
        {
            get { return enabled && m_Path != null; }
        }
        ...
        public override void MutateCameraState(ref CameraState curState, float deltaTime)
        {
            ...
            {
                var prevPos = m_PreviousPathPosition;
            ...
            var newPathPosition = m_PathPosition;

            if (deltaTime >= 0)
            {
                // Normalize previous position to find the shortest path
                var maxUnit = m_Path.MaxUnit(m_PositionUnits);
                if (maxUnit > 0)
                {
                    var prev = m_Path.NormalizeUnit(m_PreviousPathPosition, m_PositionUnits);
                    var next = m_Path.NormalizeUnit(newPathPosition, m_PositionUnits);
                ...
                var offset = m_PreviousPathPosition - newPathPosition;
            ...
            var newCameraPos = m_Path.EvaluatePositionAtUnit(newPathPosition, m_PositionUnits);
            ...
            if (deltaTime >= 0)
            {
                var currentCameraPos = m_PreviousCameraPosition;
                var delta = currentCameraPos - newCameraPos;
            ...
            else
            {
                if (deltaTime >= 0)
                {
                    var relative = (Quaternion.Inverse(m_PreviousOrientation)
        ...
        {
            var targetOrientation = m_Path.EvaluateOrientationAtUnit(m_PathPosition, m_PositionUnits);

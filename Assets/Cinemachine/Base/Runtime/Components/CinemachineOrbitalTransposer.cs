...
namespace Cinemachine
{
    ...
    public class CinemachineOrbitalTransposer : CinemachineTransposer
    {
        ...
        private class HeadingTracker
        {
            ...
            private readonly Item[] mHistory;
            private int mBottom;
            private int mCount;
            ...
            public HeadingTracker(int filterSize)
            {
                ...
                var historyHalfLife = filterSize / 5f; // somewhat arbitrarily
            ...
            private void PopBottom()
            {
                ...
                {
                    var time = Time.time;
                    var item = mHistory[mBottom];
                    if (++mBottom == FilterSize)
                        mBottom = 0;
                    --mCount;

                    var decay = Decay(time - item.time);
            ...
            {
                var time = Time.time;
                var decay = Decay(time - mWeightTime);
                mWeightSum *= decay;
                mWeightTime = time;

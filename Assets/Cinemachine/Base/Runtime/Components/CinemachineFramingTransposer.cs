...
namespace Cinemachine
{
    ...
    public class CinemachineFramingTransposer : CinemachineComponentBase
    {
        ...
        private readonly PositionPredictor m_Predictor = new PositionPredictor();
        ...
        /// <summary>
        ///     The camera will not move along its z-axis if the Follow target is within this distance of the specified camera
        ///     distance
        /// </summary>
        [Tooltip(
        ...
        [Range(0f, 1f)] [Tooltip("Camera will not move vertically if the target is within this range of the position.")]
        ...
        [Range(1, 179)] [Tooltip("If adjusting FOV, will not set the FOV higher than this.")]
        ...
        [Range(1, 179)] [Tooltip("If adjusting FOV, will not set the FOV lower than this.")]
        ...
        private float m_prevTargetHeight; // State for frame damping
        ...
        [NoSaveDuringPlay] [HideInInspector] public Action OnGUICallback;
        ...
        public Rect HardGuideRect
        {
            ...
            set
            {
                ...
                var bias = center - new Vector2(m_ScreenX, m_ScreenY);
                var biasWidth = Mathf.Max(0, m_SoftZoneWidth - m_DeadZoneWidth);
                var biasHeight = Mathf.Max(0, m_SoftZoneHeight - m_DeadZoneHeight);
        ...
        {
            get { return enabled && FollowTarget != null && LookAtTarget == null; }
        }
        ...
        public override void MutateCameraState(ref CameraState curState, float deltaTime)
        {
            ...
            TrackedPoint = m_LookaheadTime > 0
                ? m_Predictor.PredictPosition(m_LookaheadTime)
                : curState.ReferenceLookAt;
            ...
            var cameraMin = Mathf.Max(kMinimumCameraDistance, m_CameraDistance - m_DeadZoneDepth / 2);
            var cameraMax = Mathf.Max(cameraMin, m_CameraDistance + m_DeadZoneDepth / 2);
            ...
            var screenSize = curState.Lens.Orthographic
                ? curState.Lens.OrthographicSize
                : Mathf.Tan(0.5f * curState.Lens.FieldOfView * Mathf.Deg2Rad) * targetPos.z;
            var softGuideOrtho = ScreenToOrtho(SoftGuideRect, screenSize, curState.Lens.Aspect);
            if (deltaTime < 0)
            {
                // No damping or hard bounds, just snap to central bounds, skipping the soft zone
                var rect = new Rect(softGuideOrtho.center, Vector2.zero); // Force to center
            ...
            else
            {
                ...
                var hard = Vector3.zero;
                if (!m_UnlimitedSoftZone)
                {
                    var hardGuideOrtho = ScreenToOrtho(HardGuideRect, screenSize, curState.Lens.Aspect);
                    hard = OrthoOffsetToScreenBounds(targetPos, hardGuideOrtho);
                    var t = Mathf.Max(hard.x / (cameraOffset.x + Epsilon), hard.y / (cameraOffset.y + Epsilon));
                    hard = cameraOffset * t;
                }
            ...
            ref CameraState curState, float deltaTime)
        {
            ...
            var fwd = curState.RawOrientation * Vector3.forward;
            ...
            var targetHeight = GetTargetHeight(m_LastBounds);

            // Apply damping
            if (deltaTime >= 0)
            {
                var delta = targetHeight - m_prevTargetHeight;
            ...
            {
                var nearBoundsDistance = targetZ + cameraOffset - m_LastBounds.extents.z;
        ...
        {
            var framingSize = Mathf.Max(Epsilon, m_GroupFramingSize);

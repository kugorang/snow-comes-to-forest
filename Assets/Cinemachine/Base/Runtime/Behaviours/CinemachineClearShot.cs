...
namespace Cinemachine
{
    ...
    public class CinemachineClearShot : CinemachineVirtualCameraBase
    {
        ...
        private ICinemachineCamera ChooseCurrentCamera(Vector3 worldUp, float deltaTime)
        {
            ...
            var childCameras = m_ChildCameras;
            ...
            var best = LiveChild;
            foreach (var vcam in childCameras)
            {
                ...
                if (best == null
                    ...
                    || Math.Abs(vcam.State.ShotQuality - best.State.ShotQuality) < 0 && vcam.Priority > best.Priority
                    || m_RandomizeChoice && mRandomizeNow && vcam != (CinemachineVirtualCameraBase) LiveChild
                    && Math.Abs(vcam.State.ShotQuality - best.State.ShotQuality) <= 0
        ...
        private CinemachineVirtualCameraBase[] Randomize(CinemachineVirtualCameraBase[] src)
        {
            ...
            for (var i = 0; i < src.Length; ++i)
            {
                var p = new Pair();
                p.a = i;
                p.b = Random.Range(0, 1000f);
                pairs.Add(p);
            }

            pairs.Sort((p1, p2) => (int) p1.b - (int) p2.b);
            var dst = new CinemachineVirtualCameraBase[src.Length];

...
namespace Cinemachine
{
    ...
    public class CinemachineTargetGroup : MonoBehaviour
    {
        ...
        public Bounds BoundingBox
        {
            ...
            {
                float averageWeight;
                var center = CalculateAveragePosition(out averageWeight);
                var gotOne = false;
                var b = new Bounds(center, new Vector3(m_lastRadius * 2, m_lastRadius * 2, m_lastRadius * 2));
                if (averageWeight > UnityVectorExtensions.Epsilon)
                    for (var i = 0; i < m_Targets.Length; ++i)
                        if (m_Targets[i].target != null)
                        {
                            var w = m_Targets[i].weight;
                            if (w < averageWeight - UnityVectorExtensions.Epsilon)
                                w = w / averageWeight;
                            else
                                w = 1;
                            var d = m_Targets[i].radius * 2 * w;
                            var p = Vector3.Lerp(center, m_Targets[i].target.position, w);
                            var b2 = new Bounds(p, new Vector3(d, d, d));
                            if (!gotOne)
                                b = b2;
                            else
                                b.Encapsulate(b2);
        ...
        {
            var inverseView = mView.inverse;
            float averageWeight;
            var center = inverseView.MultiplyPoint3x4(CalculateAveragePosition(out averageWeight));
            var gotOne = false;
            var b = new Bounds(center, new Vector3(m_lastRadius * 2, m_lastRadius * 2, m_lastRadius * 2));
            if (averageWeight > UnityVectorExtensions.Epsilon)
                for (var i = 0; i < m_Targets.Length; ++i)
        ...
        private Quaternion CalculateAverageOrientation()
        {
            ...
            for (var i = 0; i < m_Targets.Length; ++i)
                ...
                {
                    var w = m_Targets[i].weight;
                    var q = m_Targets[i].target.rotation;

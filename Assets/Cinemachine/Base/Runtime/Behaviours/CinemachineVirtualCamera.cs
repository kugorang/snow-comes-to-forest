...
namespace Cinemachine
{
    ...
    public class CinemachineVirtualCamera : CinemachineVirtualCameraBase
    {
        ...
        {
            var oldPipeline = new List<Transform>();
            foreach (Transform child in transform)
                if (child.GetComponent<CinemachinePipeline>() != null)
                    oldPipeline.Add(child);

            foreach (var child in oldPipeline)
        ...
        private void UpdateComponentPipeline()
        {
            ...
            {
                var copyFrom = m_ComponentOwner.parent != null
            ...
            m_ComponentOwner = null;
            var list = new List<CinemachineComponentBase>();
            foreach (Transform child in transform)
                if (child.GetComponent<CinemachinePipeline>() != null)
                {
                    m_ComponentOwner = child;
                    var components = child.GetComponents<CinemachineComponentBase>();
                    foreach (var c in components)
                        list.Add(c);
                }
        ...
        private CameraState CalculateNewState(Vector3 worldUp, float deltaTime)
        {
            ...
            {
                for (var i = 0; i < m_ComponentPipeline.Length; ++i)
            ...
            var numStages = 3; //Enum.GetValues(typeof(CinemachineCore.Stage)).Length;

...
namespace Cinemachine
{
    ...
    public class CinemachineCollider : CinemachineExtension
    {
        ...
        private readonly RaycastHit[] m_CornerBuffer = new RaycastHit[4];

        private readonly Collider[] mColliderBuffer = new Collider[5];
        ...
        public List<List<Vector3>> DebugPaths
        {
            get
            {
                ...
                var extraStates = GetAllExtraStates<VcamExtraState>();
            ...
            ref VcamExtraState extra)
        {
            ...
            var distance = GetPushBackDistance(ray, startPlane, targetDistance, lookAtPos);
            if (distance <= Epsilon)
                return pos;

            // Check only as far as the obstacle bounds
            var clampedDistance = ClampRayToBounds(ray, distance, obstacle.collider.bounds);
            ...
            if (RaycastIgnoreTag(ray, out hitInfo, distance))
            {
                ...
                var adjustment = hitInfo.distance - PrecisionSlush;
                pos = ray.GetPoint(adjustment);
                extra.AddPointToDebugPath(pos);
                if (iterations > 1)
                    pos = PushCameraBack(
            ...
            Vector3 pos, Vector3 pushDir, RaycastHit obstacle, ref Vector3 outDir)
        {
            ...
            var nearbyDistance = PrecisionSlush * 5;
            var numFound = Physics.SphereCastNonAlloc(
            ...
            if (numFound > 1)
                for (var i = 0; i < numFound; ++i)
                {
                    if (m_IgnoreTag.Length > 0 && m_CornerBuffer[i].collider.CompareTag(m_IgnoreTag))
                        continue;
                    var type = m_CornerBuffer[i].collider.GetType();
        ...
        private float GetPushBackDistance(Ray ray, Plane startPlane, float targetDistance, Vector3 lookAtPos)
        {
            ...
            var angle = Mathf.Abs(Vector3.Angle(startPlane.normal, ray.direction) - 90);

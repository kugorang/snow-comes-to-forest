...
namespace Cinemachine
{
    ...
    public class CinemachineFreeLook : CinemachineVirtualCameraBase
    {
        ...
        private CameraState m_State = CameraState.Default; // Current state this frame
        ...
        {
            get { return new[] {"TopRig", "MiddleRig", "BottomRig"}; }
        }
        ...
        public override bool IsLiveChild(ICinemachineCamera vcam)
        {
            ...
            if (m_YAxis.Value < 0.33f)
                return (CinemachineVirtualCamera) vcam == m_Rigs[2];
            if (m_YAxis.Value > 0.66f)
                return (CinemachineVirtualCamera) vcam == m_Rigs[0];
            return (CinemachineVirtualCamera) vcam == m_Rigs[1];
        }
        ...
        public override void UpdateCameraState(Vector3 worldUp, float deltaTime)
        {
            ...
            if (Follow != null)
            {
                var delta = State.RawPosition - transform.position;
            ...
            var activeCam = deltaTime >= 0 || CinemachineCore.Instance.IsLive(this);
            if (activeCam)
            ...
            ICinemachineCamera fromCam, Vector3 worldUp, float deltaTime)
        {
            ...
            {
                var freeLookFrom = fromCam as CinemachineFreeLook;
        ...
        {
            var oldRigs = new CinemachineVirtualCamera[RigNames.Length];
            ...
            for (var i = 0; i < oldRigs.Length; ++i)
                if (oldRigs[i] != null)
                {
        ...
        private CinemachineVirtualCamera[] CreateRigs(CinemachineVirtualCamera[] copyFrom)
        {
            ...
            mOrbitals = null;
            var softCenterDefaultsV = new[] {0.5f, 0.55f, 0.6f};
            var newRigs = new CinemachineVirtualCamera[3];
            for (var i = 0; i < RigNames.Length; ++i)
            {
                ...
                else
                {
                    ...
                    var go = new GameObject(RigNames[i]);
                ...
                var orbital = newRigs[i].GetCinemachineComponent<CinemachineOrbitalTransposer>();
                if (orbital == null)
        ...
        private void UpdateRigCache()
        {
            ...
            if (mOrbitals != null && mOrbitals.Length == 3) return;
        ...
        private CameraState CalculateNewState(Vector3 worldUp, float deltaTime)
        {
            ...
            var state = PullStateFromVirtualCamera(worldUp);
        ...
        public Vector3 GetLocalPositionForCameraFromInput(float t)
        {
            ...
            if (mOrbitals == null) return Vector3.zero;
            UpdateCachedSpline();
            var n = 1;
            if (t > 0.5f)
            {
        ...
        private void UpdateCachedSpline()
        {
            ...
            var cacheIsValid = m_CachedOrbits != null && m_CachedTension == m_SplineCurvature;

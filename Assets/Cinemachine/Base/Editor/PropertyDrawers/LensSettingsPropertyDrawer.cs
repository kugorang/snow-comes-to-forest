...
namespace Cinemachine.Editor
{
    ...
    {
        private const int vSpace = 2;
        private readonly LensSettings def = new LensSettings(); // to access name strings
        ...
        public override void OnGUI(Rect rect, SerializedProperty property, GUIContent label)
        {
            ...
            {
                var ortho = false;
                var pi = typeof(LensSettings).GetProperty(
                    "Orthographic", BindingFlags.NonPublic | BindingFlags.Instance);
                if (pi != null)
                    ortho = Equals(true, pi.GetValue(GetPropertyValue(property), null));
        ...
        {
            var height = EditorGUIUtility.singleLineHeight + vSpace;
            if (mExpanded)
                height *= 5;
            return height;
        }
